...
\chapter{Software Requirements Specification} % Main chapter title
...
\lhead{ \emph{Software Requirements Specification}} % Change X to a consecutive number; this is for the header on each page - perhaps a shortened title
...
\begin{tabular}{| p{1.5cm} | p{2cm} | p{9cm} | p{1.5cm} |}
	...
	1.1 	& 20.10.2014		& Fix FN definition									& JR \\ \hline
	1.2 	& 23.11.2014		& Add new features filter, inspect test case		& JR \\ \hline
...
The software requirements specification provides all needed information to develop the context extraction test framework and define all delivery objects. All interfaces to external components, input and output data, deployment considerations and quality attributes are defined within this document.
...
As parts of the text extraction framework may be implemented in a server environment at a later point in time and a graphical user interface is not desired from the client, there will be no graphical user interface. The application is built, deployed and started by gradle. While the application is running, no interaction is needed.
...
\subsection{Overview}
	...
	f9  		& Filter results									& \ref{subsec:Filter results}									& nice to have \\ \hline
	f10  		& Inspect single test case							& \ref{subsec:Inspect single test case}							& nice to have\\ \hline
	\end{tabular} \\

\subsection{Filter results}


\subsection{Inspect single test case}
...
\label{subsec:Integration Boilerpipe algorithm}
	...
	\textbf{Description} 		& Boilerplate is implemented in Java. An interface is needed in order to call the Boilerplate component and get the extracted text or the extracted blocks.\\ \hline
	\textbf{Relevance} 			& needed \\ \hline
	\end{tabular} \\
...
\label{subsec:Evaluation of classification text}
	...
	\textbf{Description} 		& All the text extraction algorithms return an extracted document as text. This document needs to be checked for accuracy, which is achieved by comparing the result of the algorithms with the actual content. 
								\begin{itemize}
							        ...
							        \item Write results to an output text file
						        \end{itemize} 
...
\label{subsec:Evaluation of classification blocks}
	...
	\textbf{Description} 		& A more detailed evaluation of the algorithms could be done if not only the text but also each block of an HTML file is classified. So as to achieve the more detailed evaluation, the implementations of JusText and Boilerpiple have to be adapted so that they return classified blocks instead of the extracted text. These blocks are afterwards compared with the actual content and classified. 
								\begin{itemize}
							        ...
							        \item Insert the results in an output text file
						        \end{itemize} 
	Both the evaluation and classification are defined in more detail in section \ref{subsec:Evaluation of classification}.
	\\ \hline
	\textbf{Relevance} 			& nice to have\\ \hline
...
\label{subsec:Analyze data}
	...
	\textbf{Description} 		&  From the results of the comparison the values TP, TN, FP, FN are available. From this values further values can be evaluated for a better understanding of the results. These values are described in more detail in section \ref{subsec:Analytical values}.
								    \\ \hline
	\textbf{Relevance} 			& needed \\ \hline
	\end{tabular} \\


\subsubsection{Filter results}
\label{subsec:Filter results}

	\begin{tabular}{ | p{3cm} | p{12cm} |}
	\hline
	\textbf{Name} 				& Filter results \\ \hline
	\textbf{Feature id} 		& f9 \\ \hline
	\textbf{Description} 		&  It needs to be possible to filter the test cases based on their results. It should be possible to filter them for following values:
									\begin{itemize}
									\item Precision
									\item Recall
									\item F-Measure
									...
									To do so a line needs to be added to the configuration file. The line should be formatted like this:
									\lstinputlisting{Code/filterExample.txt}
									Now every test case where the precision value from JusText is lower than 0.8 is dropped.
...
\subsubsection{Inspect single test case}
\label{subsec:Inspect single test case}

	\begin{tabular}{ | p{3cm} | p{12cm} |}
	\hline
	\textbf{Name} 				& Inspect single test case \\ \hline
	\textbf{Feature id} 		& f10 \\ \hline
	\textbf{Description} 		&  It needs to be possible inspect a single test case more closely.
									To do so a line needs to be added to the configuration file. The line should be formatted like this:
									\lstinputlisting{Code/inspectExample.txt}
									Now the test case 'testOfInterest' is inspected more closely and an extra output file with the name 'result\_testOfInterest.txt' is generated.
									This file should contain following information.
									\begin{itemize}
									\item Statistical values (TP, FP, TN, FN, Precision, Recall, etc.)
									\item Actual content text
									\item Results from all integrated algorithms
									\item Results from all integrated formatted as blocks with the classification information for each block
									\end{itemize}
								    \\ \hline
	\textbf{Relevance} 			& nice to have \\ \hline
	\end{tabular} \\
...
The JusText algorithm is implemented in python and the documentation is found under \url{https://code.google.com/p/justext/}. See risk analysis for further information.
