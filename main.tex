...
\item ask lengi where this stupid i comes from
\item päde version: s.6 invoking???
\item aufzählung komma, komma, punkt, ok so?
\item päde version: s.14 wtf isch das für es wort
\item add toggl time tracking table
\item alles in ein repo
...
\input{Chapters/solutionDevelopment}
...
\input{Chapters/testplan}
\input{Chapters/testprotocol}
\input{Chapters/usermanual}
\input{Chapters/reports}
\input{Chapters/exampleResultsInspect}
